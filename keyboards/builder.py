"""
keyboards/builder.py — PickDeal BD v3.2 FIXED
BUG FIX: cat.get() → cat['product_count'] with try/except for sqlite3.Row
"""
from telegram import InlineKeyboardButton, InlineKeyboardMarkup


def _g(row, key, default=None):
    """Safe getter for sqlite3.Row and dict."""
    try:
        v = row[key]
...
def categories_keyboard(categories: list) -> InlineKeyboardMarkup:
    ...
    for i, cat in enumerate(categories):
        emoji = _g(cat, "emoji", "🛍️")
        count = _g(cat, "product_count", 0) or 0
        name  = _g(cat, "name", "ক্যাটাগরি")
        label = f"{emoji} {name} ({count})"
        row.append(InlineKeyboardButton(label, callback_data=f"catid_{_g(cat,'category_id',0)}"))
...
def product_list_keyboard(products: list, category_id: int, page: int,
    ...
    buttons = []
    for p in products:
        stock = int(_g(p, "stock", 0) or 0)
        price = float(_g(p, "discount_price") or _g(p, "price", 0) or 0)
        icon  = "✅" if stock > 0 else "❌"
        name  = _g(p, "name", "পণ্য")[:24]
        pid   = _g(p, "product_id", 0)
        buttons.append([InlineKeyboardButton(f"{icon} {name} — ৳{int(price):,}",
...
def product_detail_keyboard(product_id: int, category_id: int,
    ...
    wish_label = "💔 উইশলিস্ট বাদ" if in_wishlist else "❤️ উইশলিস্টে যোগ"
    buttons.append([
        InlineKeyboardButton(wish_label,              callback_data=f"wish_{product_id}"),
        InlineKeyboardButton("📤 শেয়ার",             callback_data=f"share_{product_id}"),
    ])
...
def featured_products_keyboard(products: list, back_cb: str = "browse_categories") -> InlineKeyboardMarkup:
    ...
    for p in products:
        price = float(_g(p, "discount_price") or _g(p, "price", 0) or 0)
        name  = _g(p, "name", "পণ্য")[:26]
        pid   = _g(p, "product_id", 0)
        buttons.append([InlineKeyboardButton(f"⭐ {name} — ৳{int(price):,}",
                                              callback_data=f"product_{pid}")])
...
def cart_keyboard(items: list) -> InlineKeyboardMarkup:
    buttons = []
    for item in items:
        name = _g(item, "name", "পণ্য")[:18]
        qty  = int(_g(item, "quantity", 1) or 1)
        pid  = _g(item, "product_id", 0)
        buttons.append([
            InlineKeyboardButton("➖", callback_data=f"cartqty_{pid}_dec"),
            InlineKeyboardButton(f"{name} ×{qty}", callback_data="noop"),
            InlineKeyboardButton("➕", callback_data=f"cartqty_{pid}_inc"),
            InlineKeyboardButton("🗑️", callback_data=f"cartdel_{pid}"),
        ])
    buttons.append([
...
def delivery_methods_keyboard(methods: list) -> InlineKeyboardMarkup:
    buttons = []
    for m in methods:
        charge = float(_g(m, "charge", 0) or 0)
        charge_str = f"৳{int(charge)}" if charge > 0 else "বিনামূল্যে"
        cod    = "COD✅" if _g(m, "cod_allowed") else "COD❌"
        name   = _g(m, "name", "ডেলিভারি")
        mid    = _g(m, "method_id", 0)
        buttons.append([InlineKeyboardButton(
            f"🚚 {name} ({charge_str}) {cod}",
            callback_data=f"dlv_{mid}"
        )])
...
def payment_methods_keyboard(methods: list, order_id: int) -> InlineKeyboardMarkup:
    ...
    for m in methods:
        ...
        mid   = _g(m, "method_id", 0)
        buttons.append([InlineKeyboardButton(f"{emoji} {name}",
                                              callback_data=f"pay_{order_id}_{mid}")])
...
def order_list_keyboard(orders: list) -> InlineKeyboardMarkup:
    ...
    buttons = []
    for o in orders:
        st    = short_status(_g(o, "status", "pending"))
        total = float(_g(o, "total_price", 0) or 0)
        oid   = _g(o, "order_id", 0)
        buttons.append([InlineKeyboardButton(
            f"#{oid} | {st} | ৳{int(total):,}",
            callback_data=f"vieworder_{oid}"
        )])
    buttons.append([InlineKeyboardButton("🏠 মেইন মেনু", callback_data="main_menu")])
    return InlineKeyboardMarkup(buttons)
...
def order_detail_keyboard(order_id: int, status: str) -> InlineKeyboardMarkup:
    ...
    if status == "delivered":
        buttons.append([InlineKeyboardButton("⭐ রিভিউ দিন", callback_data=f"review_{order_id}")])
    buttons.append([
...
def review_keyboard(order_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⭐ ১", callback_data=f"rev_{order_id}_1"),
         InlineKeyboardButton("⭐⭐ ২", callback_data=f"rev_{order_id}_2"),
         InlineKeyboardButton("⭐⭐⭐ ৩", callback_data=f"rev_{order_id}_3")],
        [InlineKeyboardButton("⭐⭐⭐⭐ ৪", callback_data=f"rev_{order_id}_4"),
         InlineKeyboardButton("⭐⭐⭐⭐⭐ ৫", callback_data=f"rev_{order_id}_5")],
        [InlineKeyboardButton("❌ পরে দেব", callback_data="my_orders")],
    ])
...
def wishlist_keyboard(products: list) -> InlineKeyboardMarkup:
    ...
    for p in products:
        price = float(_g(p, "discount_price") or _g(p, "price", 0) or 0)
        pid   = _g(p, "product_id", 0)
        name  = _g(p, "name", "পণ্য")[:20]
        buttons.append([
            InlineKeyboardButton(f"🛍️ {name}", callback_data=f"product_{pid}"),
            InlineKeyboardButton("💔 সরান",    callback_data=f"wish_{pid}"),
        ])
    buttons.append([InlineKeyboardButton("🏠 মেইন মেনু", callback_data="main_menu")])
    return InlineKeyboardMarkup(buttons)
...
def admin_order_actions_keyboard(order_id: int, status: str) -> InlineKeyboardMarkup:
    FLOW = {
        ...
        "processing":       [("📦 প্যাক হয়েছে","packed"),("❌ বাতিল","cancelled")],
...
def admin_product_keyboard(product_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🖼️ ছবি",         callback_data=f"admprod_image_{product_id}"),
...
def admin_products_list_keyboard(products: list, page: int, has_more: bool) -> InlineKeyboardMarkup:
    buttons = []
    for p in products:
        st    = "✅" if _g(p, "is_active") else "❌"
        price = float(_g(p, "price", 0) or 0)
        stock = int(_g(p, "stock", 0) or 0)
        name  = _g(p, "name", "পণ্য")[:20]
        pid   = _g(p, "product_id", 0)
        buttons.append([InlineKeyboardButton(
            f"{st} {name} | ৳{int(price):,} | {stock}",
            callback_data=f"admprodview_{pid}"
        )])
    nav = []
    if page > 0:
...
def admin_categories_keyboard(categories: list) -> InlineKeyboardMarkup:
    ...
    for cat in categories:
        st    = "✅" if _g(cat, "is_active") else "❌"
        emoji = _g(cat, "emoji", "🛍️")
        name  = _g(cat, "name", "ক্যাটাগরি")
        count = int(_g(cat, "product_count", 0) or 0)
        cid   = _g(cat, "category_id", 0)
        buttons.append([InlineKeyboardButton(
            f"{st} {emoji} {name} ({count})",
            callback_data=f"adm_cat_view_{cid}"
        )])
...
def admin_category_detail_keyboard(cat_id: int, is_active: bool, is_featured: bool) -> InlineKeyboardMarkup:
    toggle  = "🙈 লুকান"       if is_active  else "👁️ দেখান"
    feat    = "⭐ আন-ফিচার"   if is_featured else "⭐ ফিচার করুন"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✏️ নাম",   callback_data=f"adm_cat_edit_name_{cat_id}"),
         InlineKeyboardButton("😊 Emoji", callback_data=f"adm_cat_edit_emoji_{cat_id}")],
        [InlineKeyboardButton(toggle,      callback_data=f"adm_cat_toggle_{cat_id}"),
         InlineKeyboardButton(feat,        callback_data=f"adm_cat_feature_{cat_id}")],
...
def admin_delivery_keyboard(methods: list) -> InlineKeyboardMarkup:
    buttons = []
    for m in methods:
        st    = "✅" if _g(m, "is_active") else "❌"
        cod   = "COD✓" if _g(m, "cod_allowed") else "COD✗"
        name  = _g(m, "name", "ডেলিভারি")
        charge= float(_g(m, "charge", 0) or 0)
        mid   = _g(m, "method_id", 0)
        buttons.append([InlineKeyboardButton(
            f"{st} {name} | ৳{int(charge)} | {cod}",
            callback_data=f"adm_dlv_view_{mid}"
        )])
    buttons.append([InlineKeyboardButton("➕ নতুন ডেলিভারি", callback_data="adm_dlv_add")])
    buttons.append([InlineKeyboardButton("⬅️ Dashboard",      callback_data="adm_panel")])
    return InlineKeyboardMarkup(buttons)


def admin_delivery_detail_keyboard(method_id: int, is_active: bool) -> InlineKeyboardMarkup:
    toggle = "🙈 বন্ধ" if is_active else "✅ চালু"
    return InlineKeyboardMarkup([
...
def admin_payment_methods_keyboard(methods: list) -> InlineKeyboardMarkup:
    ...
    for m in methods:
        ...
        emoji = _g(m, "emoji", "💳")
        name  = _g(m, "name", "পেমেন্ট")
        num   = _g(m, "number", "") or "COD"
        mid   = _g(m, "method_id", 0)
        buttons.append([InlineKeyboardButton(
            f"{st} {emoji} {name} | {num}",
            callback_data=f"adm_pm_view_{mid}"
        )])
...
def admin_coupons_keyboard(coupons: list) -> InlineKeyboardMarkup:
    buttons = []
    for c in coupons:
        st   = "✅" if _g(c, "is_active") else "❌"
        val  = _g(c, "discount_val", 0) or 0
        dtype= _g(c, "discount_type", "percent")
        disc = f"{val}%" if dtype == "percent" else f"৳{int(float(val))}"
        used = _g(c, "used_count", 0) or 0
        mx   = _g(c, "max_uses", 0) or 0
        code = _g(c, "code", "—")
        cid  = _g(c, "coupon_id", 0)
        buttons.append([InlineKeyboardButton(
            f"{st} {code} | {disc} | {used}/{mx}",
            callback_data=f"adm_coupon_view_{cid}"
        )])
...
def admin_ticket_keyboard(ticket_id: int, status: str) -> InlineKeyboardMarkup:
    ...
    if status in ("open", "in_progress"):
        buttons.append([
            InlineKeyboardButton("💬 রিপ্লাই",  callback_data=f"adm_ticket_reply_{ticket_id}"),
            InlineKeyboardButton("✅ বন্ধ করুন", callback_data=f"adm_ticket_close_{ticket_id}"),
        ])
    else:
        buttons.append([InlineKeyboardButton("🔄 পুনরায় খুলুন", callback_data=f"adm_ticket_reopen_{ticket_id}")])
