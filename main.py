"""
main.py — PickDeal BD v3
Production-ready entry point. All handlers registered.
All ConversationHandlers registered BEFORE plain CallbackQueryHandlers.
"""
...
from handlers.product_handler import (
    show_categories, show_products_by_catid,
    show_product_detail, show_featured_products,
    show_bestsellers, show_new_arrivals,
...
from handlers.order_handler import (
    buy_now, checkout_from_cart, get_delivery, get_name,
    get_phone, get_address, get_notes, confirm_order, cancel_checkout,
    coupon_prompt, apply_coupon,
    DELIVERY, NAME, PHONE, ADDRESS, NOTES, CONFIRM, COUPON_INPUT,
)
from handlers.payment_handler import (
    select_payment, show_payment_instructions,
    receive_screenshot, WAITING_SCREENSHOT,
)
...
from admin.admin_handler import (
    ...
    admin_add_product_start, _ap_name, _ap_cat, _ap_desc, _ap_price,
    _ap_disc, _ap_stock, _ap_image,
    ...
    admin_view_tickets, admin_ticket_detail, admin_ticket_reply_prompt,
    admin_ticket_reply_send, admin_ticket_close, admin_ticket_reopen,
    ...
    if isinstance(update, Update) and update.effective_message:
        try:
            await update.effective_message.reply_text(
                "⚠️ একটি সমস্যা হয়েছে। /start দিয়ে আবার চেষ্টা করুন।"
            )
        except Exception:
            pass
...
def conv_checkout():
    return ConversationHandler(
        ...
        states={
            ...
            CONFIRM:  [CallbackQueryHandler(confirm_order, pattern=r"^(confirm_order|cancel_checkout)$")],
        },
        fallbacks=[
...
def conv_payment():
    return ConversationHandler(
        entry_points=[
            CallbackQueryHandler(show_payment_instructions, pattern=r"^pay_\d+_\d+$"),
        ],
        states={
...
def conv_coupon():
    return ConversationHandler(
        ...
        states={
            COUPON_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, apply_coupon)],
        },
        fallbacks=[
            CallbackQueryHandler(main_menu, pattern=r"^main_menu$"),
            CommandHandler("start", start),
...
def conv_ticket():
    return ConversationHandler(
        ...
        states={
            TICKET_MSG: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_ticket)],
        },
        fallbacks=[
...
def conv_edit_category():
    return ConversationHandler(
        ...
        states={
            EDIT_CAT_VAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, _cat_edit_save)],
        },
        fallbacks=[CommandHandler("start", start)],
...
def conv_edit_delivery():
    return ConversationHandler(
        ...
        states={
            EDIT_DLV_VAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, _dlv_edit_save)],
        },
        fallbacks=[CommandHandler("start", start)],
...
def conv_edit_payment():
    return ConversationHandler(
        ...
        states={
            EDIT_PM_VAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, _pm_edit_save)],
        },
        fallbacks=[CommandHandler("start", start)],
...
def conv_edit_coupon():
    return ConversationHandler(
        ...
        states={
            EDIT_CPN_VAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, _cpn_edit_save)],
        },
        fallbacks=[CommandHandler("start", start)],
...
def conv_ticket_reply():
    return ConversationHandler(
        ...
        states={
            TICKET_REPLY_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_ticket_reply_send)],
        },
        fallbacks=[CommandHandler("start", start)],
...
def conv_order_note():
    return ConversationHandler(
        ...
        states={
            ORDER_NOTE_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_order_note_save)],
        },
        fallbacks=[CommandHandler("start", start)],
...
def build_application() -> Application:
    if not BOT_TOKEN:
        logger.critical("❌ BOT_TOKEN সেট নেই! .env ফাইলে BOT_TOKEN দিন।")
        sys.exit(1)

    app = Application.builder().token(BOT_TOKEN).build()

    # 1. ConversationHandlers FIRST (order matters!)
    for conv in [
        ...
        conv_ticket_reply(), conv_order_note(),
    ]:
    ...
    app.add_handler(CallbackQueryHandler(admin_update_order,     pattern=r"^admupd_\d+_[\w_]+$"))
...
def main():
    logger.info("🚀 PickDeal BD v3 চালু হচ্ছে...")
    initialize_database()
    logger.info("✅ Database প্রস্তুত।")
    app = build_application()
    logger.info("✅ সব handler রেজিস্টার হয়েছে। Polling শুরু...")
