...
from handlers.start_handler import (
    ...
    review_prompt, review_star, review_text_input, REVIEW_TEXT,
)
...
from handlers.order_handler import (
    ...
    get_address, get_notes, get_payment_selection, confirm_order, cancel_checkout,
    coupon_prompt, apply_coupon,
    DELIVERY, NAME, PHONE, ADDRESS, NOTES, PAYMENT, CONFIRM, COUPON_INPUT,
)
...
def conv_checkout():
    return ConversationHandler(
        ...
        states={
            ...
            PAYMENT:  [CallbackQueryHandler(get_payment_selection, pattern=r"^pmselect_\d+$")],
            CONFIRM:  [
...
def conv_review():
    return ConversationHandler(
        entry_points=[
            CallbackQueryHandler(review_star, pattern=r"^rev_\d+_\d+$"),
        ],
        states={
            REVIEW_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, review_text_input)],
        },
        fallbacks=[CallbackQueryHandler(my_orders, pattern=r"^my_orders$"), CommandHandler("start", start)],
...
def build_application() -> Application:
    ...
    for conv in [
        conv_checkout(), conv_payment(), conv_review(), conv_search(), conv_coupon(), conv_ticket(),
